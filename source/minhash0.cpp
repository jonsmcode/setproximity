...
void minhash_sketch(const std::filesystem::path &filepath, uint64_t permutations, uint64_t minhashs[])
{
    ...
    for(auto & record : fin) {
        for(auto && kmer : record.sequence() | kmer_view) {
            std::vector<uint64_t> hashs = multi_hash(kmer, permutations);
            for(int i = 0; i < permutations; i++) {
                minhashs[i] = std::min(hashs[i], minhashs[i]);
            }
        }
    }
}
    ...
    const int permutations=100, uint64_t (*hashFunc)(uint64_t)=wyhash) 
{
    ...
    for(int i = 0; i < n; i++) {
        ...
        minhash_sketch(filepaths[i], permutations, minhashs_i);
        for(int j = i+1; j < n; j++) {
            uint64_t minhashs_j[permutations];
            minhash_sketch(filepaths[j], permutations, minhashs_j);
